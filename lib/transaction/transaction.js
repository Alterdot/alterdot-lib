...
// Burned amount and miner fee are included
Transaction.BDNS_REG_TX_FEE = 20000000;
Transaction.BDNS_UPD_TX_FEE = 1000000;
...
Transaction.prototype._hasFeeError = function (opts, unspent) {
  ...
  if (!opts.disableLargeFees) {
    ...
    var maximumFee = Math.max(maxFeePerNormalTransaction, instantSendFee, Transaction.BDNS_REG_TX_FEE / 2);
 ...
 * @param {number} amount in satoshis to be burned
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.addData = function (value, amount) {
  this.addOutput(
    new Output({
...
/**
 * BlockchainDNS transaction creation
 *
 * @param {string} prevTxId the hash of the transaction which contains the output used to pay for this transaction
 * @param {number} prevOutputIndex index of the used output from prevTxId
 * @param {number} prevSatoshis satoshis from the used output
 * @param {string} ownerAddress the one from prevTxId, owns the domain and pays for this transaction
 * @param {string} domainName
 * @param {string} content IPFS hash or other data saved under the domain
 * @param {number} type 0 for registration, 1 for update (changes the fee)
 */
Transaction.prototype.createBDNSTx = function (prevTxId, prevOutputIndex, prevSatoshis, ownerAddress, domainName, content, type) {
  var ownerAddressScript = Script.fromAddress(ownerAddress);
  var utxo = new UnspentOutput({
    txId: prevTxId,
    outputIndex: prevOutputIndex,
    address: ownerAddress,
    script: ownerAddressScript,
    satoshis: prevSatoshis
  }); // does the necessary checks on the first 4 params
  ...
  if (type === 0) {
    halfFee = Transaction.BDNS_REG_TX_FEE / 2; // half of the fees go to miners, half get burned
  } else if (type === 1) {
    halfFee = Transaction.BDNS_UPD_TX_FEE / 2;
  } else {
    throw new errors.TransaInvalidDoction.InvalidDomainTxType(
      "The BlockchainDNS transaction type can be either 0 (reg) or 1 (update)"
    );
  }

  this.fee(halfFee);

  if (typeof domainName !== 'string' || typeof content !== 'string') {
    throw new errors.Transaction.InvalidDomainOrContent(
      "The domainName and content params must be of String type"
    );
  } else if (domainName.length > 80) {
    throw new errors.Transaction.InvalidDomainOrContent(
      "The domainName param can have at most 80 characters"
    );
  } else if (content.length > 69) {
    throw new errors.Transaction.InvalidDomainOrContent(
      "The content param can have at most 69 characters"
    );
  }
  
  var returnData = "bdns/" + domainName + "/ipfs/" + content;

  this.addData(returnData, halfFee);
  this.change(ownerAddress);
